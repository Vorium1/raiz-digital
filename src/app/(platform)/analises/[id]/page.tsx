...
function RealAnalysisDetail({ analysis, delivery, canRun, canReview }: { analysis: any; delivery: DecisionDeliveryStatus | null; canRun: boolean; canReview: boolean }) {
  ...
  return <><Topbar eyebrow="Análises" title={analysis.code}><AssistantEntryButton label="Pergunte sobre esta análise"/>{canRun && <Link href={`/analises/${analysis.id}/importar`} className="button secondary"><Icon name="upload" size={16}/>Importar laudo</Link>}</Topbar><div className="content-wrap detail-page">
