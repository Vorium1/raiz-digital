...
const DIAGNOSE_FOLIAR_CONTENT = `DIAGNOSE FOLIAR DO PESSEGUEIRO E NECTARINEIRA (Tabela 6.5.17, adaptada de Freire & Magnani, 2014) -- coletar folhas completas e normais da parte mediana das brotações do ano, entre a 13ª e a 15ª semanas após a plena floração; ~100 folhas por amostra. ATUALIZAÇÃO 2026-09-04: a classificação agora está automatizada no motor (sampleType="FOLIAR") -- pra esta cultura essa tabela é ainda mais central que nas outras, porque a dose de manutenção inteira (a partir do 4º ano) é indexada só por ela, sem cruzar com solo (essa parte de DOSE continua só como texto, só a classificação do teor virou estruturada).
...
const FOLIAR_SOURCE = `Fonte: ${SOURCE_2016}, Tabela 6.5.17, p.225 -- adaptada de Freire & Magnani (2014), classes de valores pra folha de pessegueiro/nectarineira (folhas completas e normais, parte mediana das brotações do ano, 13ª-15ª semana pós-plena floração, ~100 folhas). Verificado contra o PDF oficial reextraído com \`pdftotext -table\` em 2026-09-04. Cu sem faixa de insuficiência definida no manual -- não inventada aqui.`;

/** Tabela 6.5.17 -- folha de pessegueiro/nectarineira. Sem profundidade (tecido, não solo). */
const FOLIAR_PESSEGUEIRO = [
  { parameterCode: "N", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "%", sufficiencyRanges: [{ label: "Insuficiente", max: 2.0 }, { label: "Normal", min: 3.3, max: 4.5 }, { label: "Excessivo", min: 6.0 }], criticality: "ALTA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "P", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "%", sufficiencyRanges: [{ label: "Insuficiente", max: 0.05 }, { label: "Normal", min: 0.15, max: 0.3 }, { label: "Excessivo", min: 0.4 }], criticality: "ALTA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "K", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "%", sufficiencyRanges: [{ label: "Insuficiente", max: 0.5 }, { label: "Normal", min: 1.4, max: 2.0 }, { label: "Excessivo", min: 2.8 }], criticality: "ALTA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "CA", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "%", sufficiencyRanges: [{ label: "Insuficiente", max: 0.65 }, { label: "Normal", min: 1.7, max: 2.6 }, { label: "Excessivo", min: 3.6 }], criticality: "MEDIA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "MG", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "%", sufficiencyRanges: [{ label: "Insuficiente", max: 0.2 }, { label: "Normal", min: 0.5, max: 0.8 }, { label: "Excessivo", min: 1.2 }], criticality: "MEDIA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "FE", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "mg/kg", sufficiencyRanges: [{ label: "Insuficiente", max: 50 }, { label: "Normal", min: 100, max: 230 }, { label: "Excessivo", min: 330 }], criticality: "BAIXA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "CU", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "mg/kg", sufficiencyRanges: [{ label: "Normal", min: 6, max: 30 }, { label: "Excessivo", min: 50 }], criticality: "BAIXA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "ZN", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "mg/kg", sufficiencyRanges: [{ label: "Insuficiente", max: 10 }, { label: "Normal", min: 24, max: 37 }, { label: "Excessivo", min: 50 }], criticality: "BAIXA", technicalNotes: FOLIAR_SOURCE },
  { parameterCode: "MN", parameterCategory: "QUIMICO", sampleType: "FOLIAR", depthFromCm: null, depthToCm: null, analyticalMethodAllowed: [], unitExpected: "mg/kg", sufficiencyRanges: [{ label: "Insuficiente", max: 20 }, { label: "Normal", min: 30, max: 160 }, { label: "Excessivo", min: 400 }], criticality: "BAIXA", technicalNotes: FOLIAR_SOURCE },
...
async function main() {
  ...
  await seedParameters(cropProfileId, [...P_GRUPO2, ...K_GRUPO2, ...SOLO_GERAL, S_GERAL, ...FOLIAR_PESSEGUEIRO]);
  ...
  console.log("NOTA: classificação de SOLO e de FOLHA (Tabela 6.5.17) automatizadas no motor -- dose de manutenção continua só como texto.");
}
