...
import { DashboardActivationJourney } from "@/components/dashboard-activation";
  ...
  return <>
    ...
    <div className="content-wrap dashboard-page">
      <section className="hero-panel real-hero">
        <div>
          <span className="eyebrow light">CENTRAL DE OPERAÇÕES</span>
          <h2>{priority ? `${priority} item${priority === 1 ? "" : "s"} precisa${priority === 1 ? "" : "m"} de atenção.` : "Operação sem pendências críticas."}</h2>
          <p>Os indicadores abaixo vêm direto do banco de dados real desta operação e respeitam a empresa ativa da sua sessão.</p>
          <div className="hero-actions"><Link href="/analises" className="button light">Abrir análises <Icon name="arrow" size={17}/></Link><Link href="/analises/nova" className="text-link light">Criar nova análise</Link></div>
        </div>
      ...
      <section className="metric-grid" aria-label="Indicadores reais">
        {metrics.map((metric) => <article className="metric-card" key={metric.label}><div className="metric-icon teal"><Icon name={metric.icon}/></div><div><span>{metric.label}</span><strong>{metric.value}</strong><small>{metric.detail}</small></div></article>)}
      </section>

      <section className="card analyses-card">
        <div className="card-header"><div><span className="eyebrow">FLUXO DE ANÁLISES</span><h2>Atualizações recentes</h2></div><Link href="/analises">Ver todas <Icon name="arrow" size={15}/></Link></div>
        {recent.length ? <div className="analysis-list">{recent.map((analysis) => { const meta = analysisDisplayStatus(analysis); return <Link className="analysis-row" href={`/analises/${analysis.id}`} key={analysis.id}><div className="analysis-id"><span>{analysis.code}</span><strong>{analysis.clientName}</strong><small>{analysis.fieldName} · {Number(analysis.areaHa).toLocaleString("pt-BR", { maximumFractionDigits: 2 })} ha</small></div><div className="analysis-progress"><div><i style={{ width: `${meta.progress}%` }}/></div><small>{meta.progress}% do fluxo</small></div><StatusBadge tone={meta.tone}>{meta.label}</StatusBadge><span className="analysis-updated">{formatRelativeOrDate(analysis.updatedAt)}</span><Icon name="chevron" size={18}/></Link>; })}</div> : <div className="empty-state"><Icon name="flask"/><strong>Nenhuma análise ainda</strong><small>Crie a primeira análise para alimentar esta central com dados reais.</small><Link className="button primary" href="/analises/nova">Criar análise</Link></div>}
      </section>

      <section className="system-foundation-grid">
        <article className="card foundation-card"><Icon name="shield"/><span className="eyebrow">ISOLAMENTO</span><strong>Cada empresa só vê os próprios dados</strong><small>Toda consulta ao banco roda isolada pela empresa ativa da sua sessão, aplicado no próprio banco de dados — não é um filtro de tela que dá pra contornar.</small></article>
        <article className="card foundation-card"><Icon name="history"/><span className="eyebrow">AUDITORIA</span><strong>Cadastros rastreáveis</strong><small>Criações de clientes, análises e importações registram autor, entidade e horário.</small></article>
        <article className="card foundation-card"><Icon name="upload"/><span className="eyebrow">LABORATÓRIO</span><strong>CSV validado antes de persistir</strong><small>Bloqueios técnicos mantêm a interpretação indisponível até correção humana.</small></article>
      </section>
    </div>
  </>;
}
...
function DemoDashboard() {
  ...
  return <>
    <Topbar eyebrow="Modo demonstração" title="RAIZ Digital"/>
    <div className="content-wrap dashboard-page">
      <div className="demo-banner"><Icon name="warning" size={14}/><span>Modo demonstração ativo. Números, clientes e resultados abaixo são exemplos visuais.</span></div>
