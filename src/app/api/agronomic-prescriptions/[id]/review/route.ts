...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const updated = await reviewAgronomicPrescription({ tenantId: session.tenantId, userId: session.userId, generationId: id, decision: decision as "APPROVED" | "CHANGES_REQUESTED" | "REJECTED", note: typeof body.note === "string" ? body.note : null });
