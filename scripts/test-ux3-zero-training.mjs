...
assert.match(geoMapInput, /googleMapRef\.current\.setCenter/);
assert.match(geoMapInput, /googleMapRef\.current\.setZoom\(17\)/);
assert.match(geoMapInput, /leafletMapRef\.current\?\.setView/);
assert.match(geoMapInput, /enableHighAccuracy:\s*true/);
...
assert.match(fieldVigor, /Distribuição do vigor no talhão/);
assert.match(fieldVigor, /selected\.maxNdvi/);
assert.match(fieldVigor, /selected\.zoneBreakdownPct/);
assert.match(fieldVigor, /legend=\{vigorLegend\}/);
assert.match(fieldVigor, /NDVI não é previsão direta de sacas/);
