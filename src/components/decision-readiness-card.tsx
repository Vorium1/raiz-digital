...
export function DecisionReadinessCard({ analysisId, analysisStatus, imported, interpretationStatus, delivery }: Props) {
  ...
  const stages: Stage[] = [
    {
      label: "Laudo",
      detail: imported ? "Dados recebidos" : "Aguardando entrada",
      state: imported ? "done" : "active",
    },
    {
      label: "Validação",
      detail: validationBlocked ? "Bloqueio encontrado" : imported ? "Sem bloqueio" : "Pendente",
      state: validationBlocked ? "active" : imported ? "done" : "pending",
    },
    {
      label: "Interpretação",
      detail: interpreted ? "Motor executado" : "Ainda não calculada",
      state: interpreted ? "done" : imported && !validationBlocked ? "active" : "pending",
    },
    {
      label: "Aprovação técnica",
      detail: interpretationApproved ? "Validada" : interpretationStatus === "IN_REVIEW" ? "Em revisão" : "Pendente",
      state: interpretationApproved ? "done" : interpreted ? "active" : "pending",
    },
    {
      label: "Recomendação",
      detail: recommendationApproved ? "Aprovada" : prescriptionStatus === "PENDING_REVIEW" ? "Em revisão" : prescriptionStatus === "CHANGES_REQUESTED" ? "Ajuste solicitado" : prescriptionStatus === "REJECTED" ? "Rejeitada" : "Pendente",
      state: recommendationApproved ? "done" : prescriptionStatus ? "active" : interpretationApproved ? "active" : "pending",
    },
    {
      label: "Entrega",
      detail: reportPublished ? "Relatório publicado" : "Documento pendente",
      state: reportPublished ? "done" : recommendationApproved ? "active" : "pending",
    },
  ];
  ...
  return (
    <section className={styles.card} aria-label="Prontidão da decisão técnica">
      <div className={styles.head}>
        <div>
          <span className={styles.eyebrow}>PRONTIDÃO DA DECISÃO</span>
          <h2 className={styles.title}>Do laudo à entrega técnica</h2>
          <p className={styles.description}>Uma trilha única mostra o que já está sustentado por evidência, o que depende de validação profissional e o que falta para entregar uma decisão oficial ao cliente.</p>
        </div>
        <div className={styles.score}>
          <strong>{progress}%</strong>
          <small>prontidão operacional</small>
          <StatusBadge tone={tone}>{reportPublished ? "Entrega concluída" : "Em preparação"}</StatusBadge>
        </div>
      </div>
      <div className={styles.progressTrack} aria-hidden="true"><div className={styles.progressFill} style={{ width: `${progress}%` }}/></div>
      <div className={styles.summary}>
        <div className={styles.summaryText}><strong>{headline}</strong><span>{description}</span></div>
        <Link className={styles.action} href={actionHref}>{actionLabel}<Icon name="chevron" size={12}/></Link>
      </div>
      <ol className={styles.stages}>
        {stages.map((stage, index) => (
          <li key={stage.label} className={`${styles.stage} ${stage.state === "done" ? styles.stageDone : stage.state === "active" ? styles.stageActive : ""}`}>
            <div className={styles.stageTop}><span className={styles.dot}>{stage.state === "done" ? <Icon name="check" size={11}/> : index + 1}</span><strong>{stage.label}</strong></div>
