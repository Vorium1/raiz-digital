...
/** CHUCHU (Sechium edule) -- capítulo 6.3.10 do Manual CQFS-RS/SC 2016. Grupo 2 pros dois nutrientes. Sem diagnose foliar. */
...
const NPK_CONTENT = `NITROGÊNIO -- por teor de matéria orgânica do solo: MO≤2,5%=240 kg N/ha. MO 2,6-5,0%=160. MO>5,0%=≤80. Aplicar 20kg N/ha no plantio (fonte orgânica preferencial). Restante: 10% a cada 15 dias após o início da frutificação.

FÓSFORO E POTÁSSIO -- dose por classe de disponibilidade no solo (Grupo 2 pros dois): Muito Baixo: P2O5=240, K2O=240 kg/ha. Baixo: 160/180. Médio: 120/150. Alto: 80/120. Muito Alto: ≤40/≤90. P2O5 todo no plantio. K2O: 10% a cada 15 dias após o início da frutificação.
REGRA DE DISPENSA: se P e/ou K estiver 3x acima do teor crítico, não aplicar.`;

const SOURCE_TITLE_PREFIX = `${SOURCE_2016} — capítulo Hortaliças, CHUCHU (6.3.10)`;

async function main() {
  const cropProfileId = await ensureCropProfile("CHUCHU", "Chuchu", "HORTALICA");
  console.log(`CHUCHU: ${cropProfileId}`);
