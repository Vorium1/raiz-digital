...
const PROMPT_VERSION = "prescription-gemini-v10-input-evidence-neutrality";
...
function buildPrompt(evidence: AgronomicPrescriptionEvidencePackage): string {
  return [
    ...
    "Não trate fertilizante mineral, orgânico, organomineral, inoculante, biofertilizante, remineralizador ou condicionador como superior/inferior pela categoria. Concentração NPK descreve composição; não prova eficiência agronômica total.",
    "Uma fonte de menor NPK pode ter resposta igual ou superior em contexto validado por disponibilidade, liberação, matéria orgânica, atividade biológica, crescimento radicular ou efeito residual. Mesmo assim, NUNCA converta mecanismo em crédito de nutriente ou redução de dose sem regra quantitativa homologada e rastreável para produto/mecanismo + cultura + região + dose/manejo.",
    "Registro MAPA, alegação comercial, presença de microrganismo ou ensaio laboratorial isolado não autorizam substituição quantitativa de N/P/K/S. Preserve a necessidade determinística; apresente benefícios qualitativos somente quando sustentados pela evidência recebida.",
    ...
    "`irrigationEvidence` contém o nível real de detalhe hídrico. Uma média mm/dia calculada a partir de lâmina/intervalo é somente padrão operacional, não ETc nem balanço hídrico; respeite os gates que proíbem inferência e alteração automática de dose.",
    "Parâmetros biológicos em `results` (BioAS, Azospirillum/Bradyrhizobium, micorrizas, solubilizadores de P/K, biomassa/respiração, qPCR/metabarcoding e outros ensaios microbiológicos) são evidência OPCIONAL. Ausência não é pendência. Presença, contagem ou abundância molecular não comprovam automaticamente atividade funcional nem fluxo de nutriente no campo; portanto não autorizam crédito de N/P/K/S, desconto/acréscimo de dose ou recálculo de IQS. Preserve método, unidade, táxon/grupo funcional e interpretação laboratorial; só aplique regra quantitativa quando houver motor específico homologado.",
    "`soilMicrobiologyEvidence` resume família, função, método e alertas dos parâmetros biológicos importados. Prefira esse bloco para interpretar a função e use `results` para rastreabilidade. Se o bloco proibir crédito nutricional ou ajuste de dose, não contorne a regra pela resposta textual.",
    "`biologicalSoilEvidence` é a camada BioAS específica. Preserve índices/classes oficiais importados como evidência nacional, mas NÃO recrie IQS/classes com enzimas brutas sem algoritmo oficial versionado. Enzimas sem classe continuam sendo resultados válidos e nunca bloqueiam o laudo químico-físico.",
