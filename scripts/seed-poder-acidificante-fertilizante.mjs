 ...
 *
 * ATUALIZAÇÃO (2026-09-04): a divergência DAP×MAP registrada abaixo como
 * "não resolvida" foi investigada por duas pesquisas independentes
 * (Claude e GPT, mesma rodada) e RESOLVIDA -- ver parágrafo dedicado no
 * CONTENT. Resumo: por 100kg de produto, DAP acidifica mais que MAP
 * (confirmado por 3 fontes independentes: Embrapa, FAO, RBCS 2024); por
 * kg de N entregue, a ordem se INVERTE (MAP acidifica mais que DAP) --
 * as duas afirmações são verdadeiras, medem bases de cálculo diferentes.
 * Existe ainda um terceiro eixo, não confundir com os dois acima: pH de
 * dissolução do grânulo (MAP ácido/DAP alcalino no entorno imediato,
 * efeito local e temporário, relevante pra toxidez de plântula/starter,
 * irrelevante pra acidificação residual do solo).
 */
...
const TITLE = "Poder acidificante do fertilizante × disponibilidade de nutriente por pH — pesquisa cruzada (Claude + GPT + Gemini, confiança variável por item, divergência DAP×MAP resolvida em 2026-09-04)";
const INSTITUTION = "Compilação cruzada, fonte principal: Borges & Silva, Embrapa (Fertilizantes para fertirrigação)";

const CONTENT = `ÍNDICE DE ACIDEZ/BASICIDADE (kg de CaCO3 necessários para neutralizar 100 kg do produto) -- CRUZADO entre duas fontes acadêmicas brasileiras independentes: (1) BORGES, A.L.; SILVA, D.J. Fertilizantes para fertirrigação, Cap. 7, in Irrigação e fertirrigação em fruteiras e hortaliças, Embrapa; (2) BATISTA, M.A. et al. Princípios de fertilidade do solo, adubação e nutrição mineral, in Hortaliças-fruto, EDUEM/UEM, 2018 (Tabela 18, adaptada de Tisdale, Nelson & Beaton -- referência clássica internacional). As duas fontes concordam em: Sulfato de amônio 110, Nitrato de amônio 60, Ureia ~71-84 (mesma ordem de grandeza), Nitrato de cálcio/potássio/magnésio/sódio com reação BÁSICA, KCl e sulfato de potássio NEUTROS (derruba o mito comum de que KCl acidifica solo -- a acidificação associada a lavoura potássica vem do N que acompanha a formulação, não do KCl). DIVERGÊNCIA DAP×MAP -- RESOLVIDA (2026-09-04, duas pesquisas independentes, Claude e GPT, mesma rodada, convergentes): fonte (2) tinha os valores invertidos; a versão correta é DAP=88 > MAP=60 (fonte 1, Embrapa/Borges & Silva, que já estava carregada nesta base). Confirmado por mais duas fontes internacionais independentes, todas na mesma direção (DAP mais acidificante que MAP por 100kg de produto), embora com valores absolutos diferentes por causa de teor de N do produto-referência e convenção de cálculo: FAO (DAP=74/MAP=65) e CASSIM, B.M.A.R. et al. Revisão de literatura sobre acidificação do solo por fertilizantes nitrogenados. Revista Brasileira de Ciência do Solo, 2024 (DAP=70/MAP=65). Mecanismo (por que a ordem é sempre a mesma apesar dos números absolutos variarem): a acidez vem da nitrificação do N amoniacal, não do P2O5 -- DAP tem mais N por kg de produto (~17-18%) que MAP (~11%), logo mais H+ gerado por 100kg de produto aplicado, em qualquer fonte consultada.
ATENÇÃO -- a ordem se INVERTE se a base de cálculo for "por kg de N entregue" em vez de "por 100kg de produto": usando os próprios números da Embrapa, DAP = 88÷17 ≈ 5,2 kg CaCO3/kg N, MAP = 60÷11 ≈ 5,5 kg CaCO3/kg N -- por essa base, MAP acidifica levemente MAIS que DAP. Isso é derivação (cálculo feito a partir da tabela + teor de N do rótulo), não um valor tabelado -- se for exposto em laudo, marcar como calculado e citar a base. As duas ordens (DAP>MAP por produto; MAP>DAP por N) são cientificamente corretas ao mesmo tempo -- não é contradição, é base de cálculo diferente, e o motor da RAIZ precisa declarar explicitamente qual base está usando (nunca guardar só o número "60" ou "88" sem a unidade E a base: "kg CaCO3/100kg produto" vs "kg CaCO3/kg N").
NÃO CONFUNDIR com um terceiro eixo, completamente diferente: pH de dissolução do grânulo no solo úmido, efeito local e temporário (horas/primeiros centímetros), sem relação com a acidificação residual acima -- fontes: IPNI (Nutrient Source Specifics, Diammonium Phosphate) e Incitec Pivot (MAP and DAP Fact Sheet, 2022) registram que o grânulo de MAP dissolvendo forma um microssítio ácido (pH ~3,5-4,5) e o de DAP forma um microssítio ALCALINO (pH ~7,5-8,2) por liberar amônia livre -- na direção OPOSTA da acidificação residual de longo prazo. É por isso que MAP é o "fertilizante starter" preferido em contato com semente (menor risco de toxidez amoniacal a plântula) mesmo sendo o menos acidificante no balanço residual do solo. O próprio material da Incitec Pivot e um discussion paper da consultoria SoilMate advertem que benefício agronômico de campo atribuído só à diferença de pH de dissolução raramente se confirma de forma consistente -- não transformar isso em regra agronômica de rendimento, é relevante só pra risco de toxidez de plântula/posicionamento de adubo starter, não pra escolha de programa de acidez.
