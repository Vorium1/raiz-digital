...
/** PEPINO SALADA (Cucumis sativus) -- capítulo 6.3.16 do Manual CQFS-RS/SC 2016. Grupo 2 pros dois nutrientes. Sem diagnose foliar. */
...
const NPK_CONTENT = `NITROGÊNIO -- por teor de matéria orgânica do solo: MO≤2,5%=140 kg N/ha. MO 2,6-5,0%=110. MO>5,0%=≤80. Aplicar 20kg N/ha no plantio (fonte orgânica preferencial). Restante: 10/15/15/20/20/20% aos 15/30/45/55/65/75 dias após o plantio.

FÓSFORO E POTÁSSIO -- dose por classe de disponibilidade no solo (Grupo 2 pros dois): Muito Baixo: P2O5=240, K2O=200 kg/ha. Baixo: 160/140. Médio: 120/110. Alto: 80/80. Muito Alto: ≤40/≤50. P2O5 todo no plantio. K2O: mesmo cronograma do N (10/15/15/20/20/20% aos 15-75 dias).
REGRA DE DISPENSA: se P e/ou K estiver 3x acima do teor crítico, não aplicar.`;

const SOURCE_TITLE_PREFIX = `${SOURCE_2016} — capítulo Hortaliças, PEPINO SALADA (6.3.16)`;

async function main() {
  const cropProfileId = await ensureCropProfile("PEPINO_SALADA", "Pepino salada", "HORTALICA");
  console.log(`PEPINO_SALADA: ${cropProfileId}`);
