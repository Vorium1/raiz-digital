...
for (const steadyEventName of ["gmp-steadychange", "gmp-steadystate"]) {
  const ready = new EventTarget(); let readyFailures = 0; let readySignals = 0;
  const stopReady = monitorGoogle3DHealth(ready, () => readyFailures++, 5, () => readySignals++);
  ready.dispatchEvent(Object.assign(new Event(steadyEventName), { isSteady: true }));
  ready.dispatchEvent(Object.assign(new Event(steadyEventName), { isSteady: true }));
  await new Promise(resolve => setTimeout(resolve, 15));
  assert.equal(readySignals, 1, "3D readiness must be confirmed exactly once");
...
console.log("OK — NDVI network/raster recovery, cancellation, 3D confirmed-readiness/fallback/cleanup.");
