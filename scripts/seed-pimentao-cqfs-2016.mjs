...
/** PIMENTÃO (Capsicum annuum) -- capítulo 6.3.17 do Manual CQFS-RS/SC 2016. Grupo 2 pros dois nutrientes. Sem diagnose foliar. Cronograma semanal de 17 semanas igual tomateiro/pimentão/berinjela (com fertirrigação). */
...
const NPK_CONTENT = `NITROGÊNIO -- por teor de matéria orgânica do solo: MO≤2,5%=160 kg N/ha. MO 2,6-5,0%=120. MO>5,0%=≤80. Aplicar 20kg N/ha no plantio (fonte orgânica preferencial). Restante: 4/2/2/3/4/5/6/7/8/9/9/9/8/7/6/5/4% nas semanas 1 a 17 após o plantio.

FÓSFORO E POTÁSSIO -- dose por classe de disponibilidade no solo (Grupo 2 pros dois): Muito Baixo: P2O5=280, K2O=220 kg/ha. Baixo: 200/160. Médio: 160/130. Alto: 120/100. Muito Alto: ≤80/≤70. P2O5 todo no plantio. K2O: mesmo cronograma semanal do N.
REGRA DE DISPENSA: se P e/ou K estiver 3x acima do teor crítico, não aplicar.`;

const SOURCE_TITLE_PREFIX = `${SOURCE_2016} — capítulo Hortaliças, PIMENTÃO (6.3.17)`;

async function main() {
  const cropProfileId = await ensureCropProfile("PIMENTAO", "Pimentão", "HORTALICA");
  console.log(`PIMENTAO: ${cropProfileId}`);
