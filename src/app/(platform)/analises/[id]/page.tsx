...
function RealAnalysisDetail({ analysis, delivery, canRun, canReview }: { analysis: any; delivery: DecisionDeliveryStatus | null; canRun: boolean; canReview: boolean }) {
  ...
  return <><Topbar eyebrow="Análises" title={analysis.code}><AssistantEntryButton label="Pergunte sobre esta análise"/><Link href="/analises/nova?etapa=laudo" className="button secondary"><Icon name="upload" size={16}/>Importar laudo</Link></Topbar><div className="content-wrap detail-page">
    ...
    <div className="detail-columns detail-columns-secondary"><div><InputComparisonPanel analysisId={analysis.id}/><InputApplicationsManager analysisId={analysis.id} canEdit={canRun}/></div><section className="card trace-card"><div className="card-header"><div><span className="eyebrow">RASTREABILIDADE</span><h2>Registro atual</h2></div></div><dl className="detail-list"><div><dt>ID</dt><dd>{analysis.id.slice(0,8)}…</dd></div><div><dt>Fonte</dt><dd>{analysis.sourceType || "Não definida"}</dd></div><div><dt>Importações</dt><dd>{analysis.importCount}</dd></div><div><dt>Amostras normalizadas</dt><dd>{analysis.labSampleCount}</dd></div><div><dt>Atualização</dt><dd>{formatRelativeOrDate(analysis.updatedAt)}</dd></div></dl><div className="review-actions"><small className="audit-hint"><Icon name="history" size={12}/>Mudanças de cadastro e importações são registradas na trilha de auditoria.</small></div></section></div>
  </div></>;
}
