...
// O release gate continua coberto por testes determinísticos em test-release-promotion-gate.mjs.
// A consulta às issues reais pertence exclusivamente ao workflow Production Promotion Guard,
// para que o CI técnico valide integralmente o merge commit (inclusive build) mesmo quando a
// promoção para main estiver corretamente bloqueada por gates externos.
console.log("production-readiness: cenários seguro, checkout explícito, ativação inconsistente, runtime sem admin e S3 incompleto aprovados; release gate determinístico também validado");
