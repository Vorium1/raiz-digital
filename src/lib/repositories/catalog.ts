...
export async function listAgronomicContext(tenantId: string, userId?: string) {
  return withTenant({ tenantId, userId }, async (client) => {
    const [clients, properties, fields, seasons] = await Promise.all([
      ...
      client.query(`SELECT id::text, client_id::text AS "clientId", name, municipality, state FROM properties ORDER BY name`),
      client.query(`SELECT id::text, property_id::text AS "propertyId", name, area_ha::float8 AS "areaHa" FROM fields ORDER BY name`),
  ...
  return withTenant({ tenantId: input.tenantId, userId: input.userId }, async (client) => {
    ...
    const result = await client.query(
      `WITH geom AS (
       ...
       FROM geom
       RETURNING id::text, property_id::text AS "propertyId", name, area_ha::float8 AS "areaHa"`,
      [input.tenantId, input.propertyId, input.name, boundary],
    );
    const created = result.rows[0];
    if (!created || Number(created.areaHa) <= 0) throw new Error("Geometria do talhão não produz área válida.");
