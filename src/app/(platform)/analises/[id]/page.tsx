...
function RealAnalysisDetail({ analysis, delivery, canRun, canReview }: { analysis: any; delivery: DecisionDeliveryStatus | null; canRun: boolean; canReview: boolean }) {
  ...
  return <><Topbar eyebrow="Análises" title={analysis.code}><AssistantEntryButton label="Pergunte sobre esta análise"/><Link href="/analises/nova?etapa=laudo" className="button secondary"><Icon name="upload" size={16}/>Importar laudo</Link></Topbar><div className="content-wrap detail-page">
    ...
    <DecisionReadinessCard
      ...
      imported={imported}
      interpretationStatus={analysis.latestInterpretationStatus ?? null}
      delivery={delivery}
    />

    <section id="nucleo-tecnico" className="card interpretation-card"><div className="card-header"><div><span className="eyebrow">INTELIGÊNCIA AGRONÔMICA</span><h2>Núcleo técnico da análise</h2></div>
