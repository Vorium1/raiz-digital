...
/**
 * ALFACE, ALMEIRÃO, CHICÓRIA, RÚCULA e SALSA -- capítulo 6.3.3 do Manual
 * CQFS-RS/SC 2016. O manual trata essas 5 folhosas como UM grupo de
 * recomendação só (mesma tabela de N/P/K), com pequenas diferenças de
 * cronograma de parcelamento por espécie -- bundle intencional, mesmo
 * padrão já usado pra pessegueiro+nectarineira (que também compartilham
 * uma única tabela de dose no manual). Nenhuma está nas listas oficiais
 * de Grupo 1 (P ou K) -- usa Grupo 2 pros dois.
 */
...
const NPK_CONTENT = `NITROGÊNIO -- por teor de matéria orgânica do solo (mesma tabela pras 5 espécies): MO≤2,5%=180 kg N/ha. MO 2,6-5,0%=140. MO>5,0%=≤100. Aplicar 20kg N/ha no plantio (preferencialmente fonte orgânica).
Restante por espécie -- Alface: 20/35/45% do restante aos 10/20/30 dias após o plantio. Almeirão, chicória, rúcula: 50% aos 14 dias + 50% aos 21 dias. Salsa: 25% aos 10 dias, repetindo a cada corte.

FÓSFORO E POTÁSSIO -- dose por classe de disponibilidade no solo (Grupo 2 pros dois): Muito Baixo: P2O5=240, K2O=240 kg/ha. Baixo: 160/180. Médio: 120/150. Alto: 80/120. Muito Alto: ≤40/≤90. P2O5 todo no plantio.
K2O por espécie -- Alface: 20/35/45% aos 10/20/30 dias. Almeirão, chicória, rúcula: 50% aos 14 dias + 50% aos 21 dias. Salsa: 25% aos 10 dias, repetindo a cada corte.
REGRA DE DISPENSA: se P e/ou K estiver 3x acima do teor crítico, não aplicar.`;

const SOURCE_TITLE_PREFIX = `${SOURCE_2016} — capítulo Hortaliças, ALFACE/ALMEIRÃO/CHICÓRIA/RÚCULA/SALSA (6.3.3)`;

async function main() {
  const cropProfileId = await ensureCropProfile("ALFACE", "Alface, almeirão, chicória, rúcula e salsa", "HORTALICA");
  console.log(`ALFACE: ${cropProfileId}`);
