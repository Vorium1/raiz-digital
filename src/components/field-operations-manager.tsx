      ...
      <div className="field-order-detail card">
        {!selectedOrder ? <div className="field-order-empty"><Icon name="location"/><strong>Selecione uma ordem.</strong></div> : <>
          ...
          <div className="field-point-list">
            {selectedOrder.points.slice(0, 80).map((point)=><div key={point.id} className={point.collectedAt ? "field-point-row done" : "field-point-row"}><span className="point-sequence">{point.sequence ?? "–"}</span><span><strong>{point.code}</strong><small>{point.latitude.toFixed(6)}, {point.longitude.toFixed(6)} · {point.depthFromCm}–{point.depthToCm} cm</small></span>{point.collectedAt ? <span className="point-collected"><Icon name="check" size={13}/>Coletado</span> : <button className="button tiny" disabled={busy === `collect-${point.id}`} onClick={()=>void collectHere(point)}><Icon name="location" size={13}/>{busy === `collect-${point.id}` ? "GPS…" : "Confirmar aqui"}</button>}</div>)}
