...
type CropProfile = { id: string; code: string; name: string; semanticVersion: string; status: "DRAFT" | "ACTIVE" | "SUPERSEDED"; technicalNotes: string | null };
...
export function TechnicalLibraryManager({ referenceUnits }: { referenceUnits: Record<string, string> }) {
  ...
  async function loadAll() {
    const [profilesRes, regionsRes, ruleSetsRes] = await Promise.all([
  ...
  async function createCrop() {
    ...
    try {
      await postJson("/api/crop-profiles", { code: newCropCode, name: newCropName });
      setNewCropCode(""); setNewCropName("");
  ...
  return (
    <div className="field-ops-stack">
      ...
      <details open><summary><span><b>1</b><strong>Culturas</strong><small>Catálogo versionado — nunca faixa inventada</small></span><Icon name="chevron" size={16}/></summary>
